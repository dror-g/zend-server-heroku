...
echo "UID IS:"
echo $ZEND_UID
echo "WEB_USER=u${ZEND_UID}" >> /app/etc/zce.rc

ZS_MANAGE=/app/zend-server-6-php-5.4/bin/zs-manage

## NO EFFECT!!!!
echo "Setting NGINX VHOST PORT"
#sed -i "s/VCAP_PORT/${PORT}/" /app/nginx/conf/sites-available/default
#cat /app/nginx/conf/sites-available/default|sed "s/VCAP_PORT/${PORT}/" > /app/nginx/conf/sites-available/default
...
echo "server {" > /app/nginx/conf/sites-enabled/default
echo "      listen   ${PORT} default;" >> /app/nginx/conf/sites-enabled/default
cat /app/nginx/conf/sites-available/default.erb >> /app/nginx/conf/sites-enabled/default
#echo "DONE ${?}"
